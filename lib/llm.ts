...
async function once(
  ...
  try {
    const res = await fetch(ENDPOINT, {
      ...
      // Several of the free models are reasoning models: without a large
      // budget the whole allowance goes into the reasoning trace and content
      // comes back null with finish_reason "length". Low effort keeps the
      // trace short enough to leave room for an actual answer.
      body: JSON.stringify({
        model,
        messages,
        temperature: 0.4,
...
export async function chat(
  ...
  // A model that answered well but ignored the JSON envelope has still done the
  // work. Keep the best prose we saw and fall back to it rather than throwing
  // away a good answer over formatting.
  let salvage: string | null = null;

  for (const model of MODELS) {
    const out = await once(model, messages, opts.maxTokens ?? 2600, key);
    if (!out) continue;
    if (!opts.json) return out;
    if (parseJson(out) !== null) return out;
    if (!salvage && out.trim().length > 60 && !out.trim().startsWith("{"))
      salvage = out.trim();
  }
  return salvage;
}
