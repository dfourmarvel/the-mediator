...
export function computeBalance(
  ...
  if (floorsTotal > yield_) {
    const scale = yield_ / floorsTotal;
    return {
      yield_,
      floorsTotal,
      pool: 0,
      ...
      shortfall: eligible
        ...
        .map((p) => ({
          partyId: p.id,
          missing: Math.round(p.floor - p.floor * scale),
        })),
      allocations: eligible.map((p) => ({
        partyId: p.id,
        floor: Math.round(p.floor * scale),
        discretionary: 0,
        total: Math.round(p.floor * scale),
        ofClaim: (p.floor * scale) / p.claim,
      })),
      gated,
    };
  }
  ...
  for (const p of eligible) {
    need.set(p.id, Math.max(0, p.claim - p.floor));
    award.set(p.id, 0);
  }
